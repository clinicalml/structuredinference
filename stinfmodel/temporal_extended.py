...
class DKF(BaseModel, object):
    ...
    def _createGenerativeParams(self, npWeights):
        ...
        if self.params['transition_type']=='mlp':
            ...
            if self.params['use_prev_input']:
                ...
                #Sum
                #npWeights['p_inp_embed_W_0'] = self._getWeight((self.params['dim_observations'], DIM_HIDDEN_TRANS))
                #npWeights['p_inp_embed_b_0'] = self._getWeight((DIM_HIDDEN_TRANS,))
                #then add to gate to use for gating
                #Concat 
        ...
        npWeights['p_trans_W_logsigsq'] = self._getWeight((MU_COV_INP, self.params['dim_stochastic']))
        npWeights['p_trans_b_logsigsq'] = self._getWeight((self.params['dim_stochastic'],))
        ...
        if self.params['emission_type'] == 'mlp':
            ...
            for l in range(self.params['emission_layers']):
                ...
                npWeights['p_emis_W_'+str(l)] = self._getWeight((dim_input, dim_output))
                npWeights['p_emis_b_'+str(l)] = self._getWeight((dim_output,))
            #dim_input,dim_output = self.params['dim_observations'], DIM_HIDDEN
            #npWeights['p_inp_W'] = self._getWeight((dim_input, dim_output))
            #npWeights['p_inp_b'] = self._getWeight((dim_output,))
    ...
    def _createInferenceParams(self, npWeights):
        ...
        npWeights['q_W_logsigsq'] = self._getWeight((RNN_SIZE, self.params['dim_stochastic']))
        npWeights['q_b_logsigsq'] = self._getWeight((self.params['dim_stochastic'],))
        assert self.params['lr_combine']=='split','Expecting split'
        if self.params['var_model']=='lstmlr' and self.params['inference_model']=='mean_field':
            ...
            npWeights['q_W_logsigsq_r'] = self._getWeight((RNN_SIZE, self.params['dim_stochastic']))
            npWeights['q_b_logsigsq_r'] = self._getWeight((self.params['dim_stochastic'],))
    ...
    def _getEmissionFxn(self, z, X=None):
        ...
        if self.params['emission_type']=='mlp':
            ...
            #assert X is not None,'Expecting X'
            ...
            #import ipdb;ipdb.set_trace()
            #
            #hid2    = self._LinearNL(self.tWeights['p_inp_W'], self.tWeights['p_inp_b'], X_prev)
            #import ipdb;ipdb.set_trace()
            #hid = hid+hid2
        elif self.params['emission_type']=='conditional_linear':
        ...
        if self.params['data_type']=='binary':
            ...
            print x_reshaped.tag.test_value.shape
            #Set initial states of NADE 
        ...
        if self.params['transition_type']=='simple_gated':
            def mlp(inp, W1,b1,W2,b2,X_prev=None,W_prev=None,b_prev=None):
                ...
                if X_prev is not None:
                    #Add
                    #h1 = self._LinearNL(W1,b1, inp) +  self._LinearNL(W_prev,b_prev, X_prev)
                    #Concatenate
            ...
            if self.params['use_prev_input']:
                ...
                X_prev = T.concatenate([T.zeros_like(X[:,[0],:]),X[:,:-1,:]],axis=1)
                #If concat, ignore these
                #W_prev = self.tWeights['p_inp_embed_W_0']
                #b_prev = self.tWeights['p_inp_embed_b_0']
            ...
            cov    = T.nnet.softplus(T.dot(self._applyNL(z_prop), self.tWeights['p_trans_W_logsigsq'])+
                                     self.tWeights['p_trans_b_logsigsq'])
            return mu,cov
            ...
            for l in range(self.params['transition_layers']):
                ...
                hid = self._LinearNL(self.tWeights['p_trans_W_'+str(l)],
                                     ...
                                     hid)
                #if l==0 and self.params['use_prev_input']:
                #    hid+=self._LinearNL(self.tWeights['p_inp_embed_W_0'],self.tWeights['p_inp_embed_b_0'],X)
            mu     = self._LinearNL(self.tWeights['p_trans_W_mu'],
                                      self.tWeights['p_trans_b_mu'],hid,onlyLinear=True)
            cov    = T.nnet.softplus(T.dot(hid, self.tWeights['p_trans_W_logsigsq'])+
                                     self.tWeights['p_trans_b_logsigsq'])
            return mu,cov
        else:
    ...
    def _inferenceLayer(self, hidden_state, eps):
        ...
        def structuredApproximation(h_t, eps_t, z_prev, 
            ...
            cov_t        = T.nnet.softplus(T.dot(h_next,q_W_logsigsq)+q_b_logsigsq)
            z_t          = mu_t+T.sqrt(cov_t)*eps_t
            return z_t, mu_t, cov_t
        
        if self.params['var_model']=='lstmlr':
            assert type(hidden_state) is list,'Expecting list'
            assert self.params['lr_combine']=='split','expecting sp. l->r and r->l'
            if self.params['inference_model']=='structured':
                eps_swap= eps.swapaxes(0,1)
                rval, _ = theano.scan(structuredApproximation, 
                                        sequences=[hidden_state[0]+hidden_state[1], eps_swap],
                                        outputs_info=[
                ...
                cov_1    = T.nnet.softplus(T.dot(hidl2r,self.tWeights['q_W_logsigsq'])+self.tWeights['q_b_logsigsq'])

                hidr2l = r2l
                mu_2     = T.dot(hidr2l,self.tWeights['q_W_mu_r'])+self.tWeights['q_b_mu_r']
                cov_2    = T.nnet.softplus(T.dot(hidr2l,self.tWeights['q_W_logsigsq_r'])+self.tWeights['q_b_logsigsq_r'])
            ...
            if self.params['inference_model']=='mean_field':#MF-R,MF-L
                ...
                z         = mu + T.sqrt(cov)*eps
            elif self.params['inference_model']=='structured': #ST-R/ST-L
                eps_swap= eps.swapaxes(0,1)
                rval, _ = theano.scan(structuredApproximation, 
